...
final class FakeFetch
{
    ...
    public function handle(string $bridge, array $payload): array
    {
        ...
        $this->recordEvent(new FetchRequestStarted($request->requestId(), $request->method(), $request->url()));
        $response = $this->responseFor($request)->withRequestId($request->requestId());

        if ($bridge === 'Fetch.Download') {
            $this->recordEvent(new FetchDownloadCompleted(
                $request->requestId(),
                $response->status(),
                $response->headers(),
            ...
            $this->recordEvent(new FetchRequestCompleted(
                $response->requestId(),
                $response->status(),
                $response->headers(),
                $response->body(),
    ...
    public function assertNotSent(?Closure $callback = null): void
    {
        ...
        if ($matched) {
            throw new \RuntimeException('An unexpected Fetch request was sent.');
        }
    }

    public function assertSentCount(int $count): void
    {
        if (count($this->requests) !== $count) {
            throw new \RuntimeException("Expected {$count} Fetch requests; recorded ".count($this->requests).'.');
        }
    }
    ...
    private function responseFor(RecordedRequest $request): FetchResponse
    {
        foreach ($this->responses as $pattern => $response) {
            if (Str::is($pattern, $request->url())) {
                ...
                if ($resolved instanceof FetchResponse) {
                    return $resolved;
                }
                if (is_array($resolved)) {
                    return FetchResponse::make(body: $resolved);
                }
