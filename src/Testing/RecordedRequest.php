...
final readonly class RecordedRequest
{
    ...
    {
        return (string) ($this->payload['request_id'] ?? '');
    }

    public function method(): string
    {
        return (string) ($this->payload['method'] ?? ($this->bridge === 'Fetch.Download' ? 'GET' : ''));
    }

    public function url(): string
    {
        return (string) ($this->payload['url'] ?? '');
    }

    public function headers(): array
    {
        return (array) ($this->payload['headers'] ?? []);
    }

    public function body(): ?array
    {
        return isset($this->payload['body']) && is_array($this->payload['body']) ? $this->payload['body'] : null;
    }
