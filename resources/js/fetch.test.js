...
test("builds downloads and cancellation with a pre-existing ID", async () => {
  ...
  await request.download("https://example.test/file.pdf", "/app/file.pdf", {
    query: { version: 2 },
    overwrite: true,
  });
...
test("supports form and raw body modes", async () => {
  await Fetch.asForm().post("https://example.test", {
    name: "Victory Efe",
    symbols: "a&b=c",
    enabled: true,
    tags: ["a", "b"],
  });
  await Fetch.withBody("<user>Victory</user>", "application/xml").post(
    "https://example.test",
  );
  assert.equal(calls[0].params.body.type, "form");
  assert.equal(
    calls[0].params.body.data,
    "name=Victory+Efe&symbols=a%26b%3Dc&enabled=1&tags=a&tags=b",
  );
  assert.deepEqual(calls[1].params.body, {
    type: "raw",
    data: "<user>Victory</user>",
  });
  assert.equal(calls[1].params.headers["Content-Type"], "application/xml");
});

test("rejects incompatible body modes", () => {
  assert.throws(
    () => Fetch.attach("file", "/tmp/a").asForm(),
    /cannot be combined/,
  );
  assert.throws(
    () => Fetch.withBody("raw").attach("file", "/tmp/a"),
    /cannot be combined/,
  );
  assert.throws(
    () => Fetch.withBody("raw").post("https://example.test", { extra: true }),
    /method data/,
  );
  assert.throws(() => Fetch.withBody(42), /must be a string/);
});
